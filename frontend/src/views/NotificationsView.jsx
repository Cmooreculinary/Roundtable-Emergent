...
export default function NotificationsView({ notifications = [], onRefresh }) {
  ...
  const markAll = async () => {
    if (busy || notifications.length === 0) return;
    setBusy(true);
    try {
      ...
      toast.error(formatApiErrorDetail(error.response?.data?.detail) || "Could not mark notifications read");
    } finally {
      setBusy(false);
    }
  };
  ...
  const clearAll = async () => {
    if (busy || notifications.length === 0 || !window.confirm("Move all notifications to trash?")) return;
    setBusy(true);
    try {
      ...
      toast.error(formatApiErrorDetail(error.response?.data?.detail) || "Could not clear notifications");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      <div className="audit-page-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, gap: 10 }}>
        ...
        <div className="audit-action-group">
          {hasUnread && <button className="btn btn-secondary" type="button" onClick={markAll} disabled={busy} data-testid="notif-mark-all"><Check size={14} /> Mark all read</button>}
          {notifications.length > 0 && <button className="btn btn-secondary" type="button" onClick={clearAll} disabled={busy} data-testid="notif-clear-all" style={{ color: "var(--mac-red)" }}><Trash2 size={14} /> Clear All</button>}
        </div>
      </div>
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }} aria-live="polite">
          {notifications.map((n) => (
            <div key={n.id} style={{ padding: 12, display: "flex", gap: 10, alignItems: "start", borderBottom: "1px solid var(--border-light)", background: n.read ? "transparent" : "rgba(0,122,255,0.04)" }} data-testid={`notif-${n.id}`}>
              ...
              {!n.read && <span aria-label="Unread" style={{ width: 8, height: 8, borderRadius: 8, background: "var(--mac-blue)", marginTop: 6 }} />}
            </div>
          ))}
        </div>
      )}
